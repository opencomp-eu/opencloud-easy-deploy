...
def update_spec(project_root: Path | None = None) -> UpdateSpec:
    ...
    state_dir = root / ".opencloud-easy-deploy"
    return UpdateSpec(
        project_root=root,
        state_dir=state_dir,
        config_paths=(root / "deploy.yaml", *integration_config_paths(state_dir)),
        compose_projects=("opencloud-compose",),
        extra_containers=("opencloud", "ldap-server", "opencloud_caddy"),
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Update opencloud-easy-deploy")
